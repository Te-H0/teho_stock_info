...
- **수급**(KR close): 외국인/기관/개인 시장 전체 + 담은(🟢)/판(🔴) 섹터 + 대장주 종목별
- **운영**: 맥 Docker(`docker-compose.yml` restart:always, `scheduler.py`). 발송 — morning(미국) 화~토 07:30 / close(한국) 월~금 16:00. 네트워크 재시도 2단(toss 종목별 + scheduler 10초×3) + 실패 시 IP 텔레 알림
